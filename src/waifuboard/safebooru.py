...
class SafebooruPosts(SafebooruComponent):
    ...
    async def list(
        ...
        # 获取当前查询标签下所有页码的帖子列表
        if all_page:
            max_pid = await self.list_pid(
                tags=tags,
            )  # 获取 html 分页器中的最大 pid
            max_pid += 42  # max_pid 仅代表当前最大页码的第一篇帖子的上一个序号，加上固定的每页 42 篇帖子，得到当前页码的最后一篇帖子序号的边界范围
            max_page = (
                max_pid // limit
            )  # 计算最大页码（保守计算，以免超出限制引发响应异常）
            logger.info(
                f"Maximum page number is equal to {max_page} for {limit = }, {tags = }"
            )

            # 查看整除计算结果是否忽略了某些帖子，以及能否在不超过最大帖子数量限制的情况下获取它们
            ignored_posts: bool = (max_pid - max_page * limit > 0) and (
                (max_page + 1) * limit <= self.client.MAX_PID
            )
            ...
            if max_pid > self.client.MAX_PID:
                ...
                logger.warning(
                    f"Maximum page number is set to {max_page} for {limit = }, {tags = }, because {max_pid + remain_pid} exceeds {self.client.MAX_PID}"
                )
        ...
        # 获取在起始页码与结束页码范围内，指定标签的帖子列表
            ...
            if (start_pid := limit * start_page) > self.client.MAX_PID:
                start_page = (
                    self.client.MAX_PID // limit
                )  # 计算最大页码（保守计算，以免超出限制引发响应异常）
                logger.warning(
                    f"Start page is set to {start_page} for {limit = }, {tags = }, because {start_pid} exceeds {self.client.MAX_PID}"
                )
