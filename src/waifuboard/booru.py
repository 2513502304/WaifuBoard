    ...
    async def request(
        ...
        *,
        ...
        Args:
            ...
            expected_statuses (Collection[int], optional): HTTP statuses that should be returned as expected business states without triggering an outer status retry or proxy failure. Matched statuses are returned immediately; unmatched 4xx/5xx responses consume the available outer attempts, but the final response is returned without raising HTTPError when the budget is exhausted. This does not override niquests' inner Retry status_forcelist. Defaults to None.
